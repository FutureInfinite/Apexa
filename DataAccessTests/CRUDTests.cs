...
namespace ApexaTests
{
    public class CRUDTests
    {
        ...
        {
            //Prepare DB DI            
            DependencyPreparation.PrepareDependencies(SystemAccessOps.Services);
            SystemAccessOps.BuildServices();            
            AdRepo = SystemAccessOps.Provider.GetRequiredService<IAdvisorRepository>();            
        }
        #endregion Lifetime
        ...
        private async Task CreateAdvisor(Advisor Adv)
        {
            if (AdRepo != null)
            {
                try
                {
                    await AdRepo.CreateAdvisor(Adv);                    
                }
                catch (Exception ex)
                {
        ...
        private async Task DeleteAdvisor(string SIN)
        {
            ...
            {                                          
                ...
                await AdRepo.DeleteAdvisor(SIN);                
            }
            catch (Exception ex)
            {
                Assert.Fail(ex.Message);
        ...
        public async void CreateDeleteAdvisor()
        {
            ...
            if (AdRepo != null)
            {
                try
                {
                    ...
                    //get the advisro --
                    RetrievedAdvisor = await AdRepo.GetAdvisor(NewAdvisor1.SIN);
                ...
                {
                    if (RetrievedAdvisor != null) await DeleteAdvisor(RetrievedAdvisor.SIN);
                }
            }
        ...
        public async void UpdateAdvisor()
        {
            ...
            try
            {
                ...
                RetrievedAdvisor.HealthStatus = HoldHS ==
                                                ...
                                                HealthStatus.Yellow;
                //save thge changes
                await AdRepo.SaveAdvisor(RetrievedAdvisor);

                //validate changes have taken place
                UpdatedAdvisor = await AdRepo.GetAdvisor(RetrievedAdvisor.SIN);
        ...
        public async void GetAdvisors()
        {
            try
            {
                ...
                IEnumerable<Advisor> AllAdvisors = await AdRepo.GetAdvisors();
        ...
        public async void ValidateUniqueSIN()
        {
            ...
            {
               await DeleteAdvisor(NewAdvisor1.SIN);
            }
        }
